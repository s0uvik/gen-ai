...
client = OpenAI()
grok = OpenAI(
    base_url="https://api.x.ai/v1", api_key=os.getenv("XAI_API_KEY")  # xAI endpoint
)
...
while True:
    response = client.chat.completions.create(
        model="gpt-4.1-mini", response_format={"type": "json_object"}, messages=messages
    )
    ...
    #     # Make a Grok API call
    #     grok_response = grok.chat.completions.create(
    #         model="grok-beta",   # latest Grok chat model
    #         messages=[
    #             {"role": "system", "content": "You are Grok, validating OpenAI's reasoning."},
    #             {"role": "user", "content": parsed_response.get("content", query)}
    #         ],
    #         max_tokens=500
    #     )

    #     # Extract Grok’s output
    #     grok_text = grok_response.choices[0].message.content

    #     # Append Grok’s validation back into OpenAI’s conversation
    #     messages.append({
    #         "role": "assistant",
    #         "content": f"<Grok validation> {grok_text}"
    #     })
